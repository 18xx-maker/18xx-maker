...
const Private = (props) => {
  ...
  if (players) {
    let minNumPlayers = reduce(
      min,
      99,
      map((p) => p.number, players),
    );
    let maxNumPlayers = reduce(
      max,
      0,
      map((p) => p.number, players),
    );
  ...
  const icons = (
    <>
      {hexNode}
      {company && (
        <div className="private__company">
      ...
      {icon && (
        <div className="private__icon">
          <Svg viewBox="-15 -15 30 30">
            <Icon
              type={icon}
              color={iconColor}
              strokeWidth={style === "big" ? 1 : 2}
            />
          </Svg>
        </div>
      )}
            ...
            {(c) => (
              ...
              >
                <div className="card__body">
                  ...
                  {style === "big" && icons}
                  <div
                    className="private__description"
                    style={{
                      color: c(descColor),
