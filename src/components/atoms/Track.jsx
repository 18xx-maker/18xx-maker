...
const startEndDeprecated = (type, replace, end, start) => {
  console.log(
    `Track type "${type}" is currently deprecated. The same effect can be made with the new "start" and "end" fields on track: {"type": "${replace}"${start ? `, "start": ${start}` : ""}${end ? `, "end": ${end}` : ""}}`,
  );
};
...
const arcPath = (start, end, xOffset, arcRadius, arcAngle, hexAngle) => {
  ...
  let [bleedEndX, bleedEndY] = rotatePoint(
    CENTER_EDGE_X - xOffset,
    beginY + BLEED,
    -hexAngle,
  );
...
const Track = ({
  ...
  if (start === 0 && begin !== 0) start = begin;
  if (end === 1 && stop !== 1) end = stop;
  ...
  if (border) trackWidth += borderWidth;
  gauge = game.info.trackGauge || gauge;
  // and any "match" colors which uses the hex color instead
  color = multiDefaultTo(
    "track",
    color === "match" ? bgColor : color,
    game.info.trackColor === "match" ? bgColor : game.info.trackColor,
  );
  borderColor = multiDefaultTo(
    "border",
    borderColor === "match" ? bgColor : borderColor,
    game.info.trackBorderColor === "match"
      ? bgColor
      : game.info.trackBorderColor,
  );
  gaugeColor = multiDefaultTo(
    "white",
    gaugeColor === "match" ? bgColor : gaugeColor,
      ...
      {(c) => (
        <g transform={`rotate(${rotation})`}>
          <path
            d={path}
            fill={
              type === "offboard" ? c(border ? borderColor : color) : "none"
            }
            stroke={
              type === "offboard" ? "none" : c(border ? borderColor : color)
            }
            strokeLinecap="butt"
            strokeLinejoin="miter"
