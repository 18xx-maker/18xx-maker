...
const Border = ({ border, data, bg }) => {
  let path =
    "M " + map((coord) => mapCoord(coord, data), border.coords).join(" L ");

  let width = (border.width || 8) * data.scale;
  let borderWidth = border.borderWidth
    ? border.borderWidth * data.scale
    : width + 4 * data.scale;
...
const Borders = ({ data }) => {
  let borders = addIndex(map)(
    (b, i) => (
      <Border key={`border-bg-${i}`} bg={true} border={b} data={data} />
    ),
    data.borders || [],
  );
  borders = concat(
    borders,
    addIndex(map)(
      (b, i) => <Border key={`border-${i}`} border={b} data={data} />,
      data.borders || [],
    ),
  );
  return borders;
