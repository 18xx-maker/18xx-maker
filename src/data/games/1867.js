const game = {
  // Generic Game Info
  info: {
    title: "1867 (WIP)",
    designer: "Ian Wilson",
    background: "water",
    width: 150,
    orientation: "horizontal",
    titleX: 1200,
    titleY: 1200,
    color_5: "cyan",
    color_10: "blue"
  },

  tokens: ["Round"],

  ipo: true,

  bank: "¥7,000",

  players: [
    {
      number: 2,
      certLimit: 25,
      capital: "¥420"
    },
    {
      number: 3,
      certLimit: 19,
      capital: "¥420"
    },
    {
      number: 4,
      certLimit: 14,
      capital: "¥420"
    },
    {
      number: 5,
      certLimit: 12,
      capital: "¥390"
    },
    {
      number: 6,
      certLimit: 11,
      capital: "¥390"
    }
  ],

  pools: [
    {
      name: "Bank Pool",
      notes: [
        {
          color: "blue",
          note: "Shares in the market pay dividends to the corporation"
        },
        {
          color: "purple",
          icon: "exclamation",
          note:
          "No more than 50% of a corporation's shares may be in the market at any time"
        },
        {
          color: "red",
          icon: "times",
          note: "Shares cannot be sold during the first stock round"
        }
      ]
    }
  ],

  rounds: [
    {
      name: "OR3",
      color: "brown"
    },
    {
      name: "OR2",
      color: "green"
    },
    {
      name: "OR1",
      color: "yellow"
    },
    {
      name: "SR",
      color: "gray"
    }
  ],

  turns: [
    {
      name: "Stock Round",
      steps: ["Buy one certificate", "Sell any number of certificates"],
      ordered: false
    },
    {
      name: "Operating Round",
      steps: [
        "Lay or upgrade track",
        "Purchase a station",
        "Run trains",
        "Pay dividends or withhold revenue",
        "Purchase trains"
      ],
      ordered: true
    }
  ],

  stock: {
    type: "2D",
    par: {
      values: [100, 90, 80, 75, 70, 65]
    },
    movement: {
      up: ["Sold out"],
      down: ["Every share sold"],
      left: ["Withheld revenue"],
      right: ["Paid dividends"]
    },
    market: [
      [
        { label: "75", arrow: "down" },
        80,
        90,
        { label: "100", par: true },
        110,
        125,
        140,
        155,
        175,
        200,
        225,
        255,
        285,
        315,
        350
      ],
      [
        { label: "70", arrow: "down" },
        75,
        80,
        { label: "90", par: true },
        100,
        110,
        125,
        140,
        155,
        175,
        200,
        225,
        255,
        285,
        { label: "315", arrow: "up" }
      ],
      [
        { label: "65", arrow: "down" },
        70,
        75,
        { label: "80", par: true },
        90,
        100,
        110,
        125,
        140,
        155,
        175,
        { label: "200", arrow: "up" }
      ],
      [
        { label: "60", arrow: "down" },
        65,
        70,
        { label: "75", par: true },
        80,
        90,
        100,
        110,
        125,
        { label: "140", arrow: "up" }
      ],
      [
        { label: "55", arrow: "down" },
        60,
        65,
        { label: "70", par: true },
        75,
        80,
        90,
        { label: "100", arrow: "up" }
      ],
      [
        { label: "50", legend: 0, arrow: "down" },
        55,
        60,
        { label: "65", par: true },
        70,
        75,
        { label: "80", arrow: "up" }
      ],
      [
        { label: "45", legend: 0, arrow: "down" },
        { label: "50", legend: 0 },
        55,
        60,
        65,
        { label: "70", arrow: "up" }
      ],
      [
        { label: "40", legend: 0, arrow: "down" },
        { label: "45", legend: 0 },
        { label: "50", legend: 0 },
        55,
        { label: "60", arrow: "up" }
      ],
      [
        { label: "30", legend: 1, arrow: "down" },
        { label: "40", legend: 0 },
        { label: "45", legend: 0 },
        { label: "50", legend: 0, arrow: "up" }
      ],
      [
        { label: "20", legend: 1, arrow: "down" },
        { label: "30", legend: 1 },
        { label: "40", legend: 0 },
        { label: "45", legend: 0, arrow: "up" }
      ],
      [
        { label: "10", legend: 1 },
        { label: "20", legend: 1 },
        { label: "30", legend: 1 },
        { label: "40", legend: 0, arrow: "up" }
      ]
    ],
    legend: [
      {
        color: "cyan",
        description:
        "Shares of this corporation do not count toward the certificate limit",
        icon: "certificate"
      },
      {
        color: "blue",
        description: "Players may own more than 60% of this corporation",
        icon: "percentage"
      }
    ]
  },

  trains: [
    {
      name: "2",
      quantity: 6,
      price: "¥80",
      color: "yellow",
      info: [
        {
          color: "green",
          note: "Rusted by 4"
        }
      ]
    },
    {
      name: "3",
      quantity: 5,
      price: "¥180",
      color: "green",
      info: [
        {
          color: "brown",
          note: "Rusted by 6"
        }
      ]
    },
    {
      name: "4",
      quantity: 4,
      price: "¥300",
      color: "green",
      info: [
        {
          color: "brown",
          note: "Rusted by D"
        }
      ]
    },
    {
      name: "5",
      quantity: 3,
      price: "¥450",
      color: "brown",
      info: [
        {
          color: "yellow",
          note: "Permanent"
        }
      ]
    },
    {
      name: "6",
      quantity: 2,
      price: "¥630",
      color: "brown",
      info: [
        {
          color: "yellow",
          note: "Permanent"
        }
      ]
    },
    {
      name: "D",
      quantity: 9,
      price: "¥1100",
      color: "brown",
      description: "Cost ¥800 when trading in a 4T, 5T or 6T",
      info: [
        {
          color: "yellow",
          note: "Permanent"
        }
      ]
    }
  ],

  companies: [
    {
      name: "Awa Railroad",
      abbrev: "AR",
      color: "red",
      tokens: ["Free", "¥40"],
      shares: [
        {
          quantity: 1,
          label: "President Share",
          percent: 20,
          shares: 2
        },
        {
          quantity: 8,
          percent: 10,
          shares: 1
        }
      ]
    },
    {
      name: "Iyo Railway",
      abbrev: "IR",
      color: "orange",
      tokens: ["Free", "¥40"],
      shares: [
        {
          quantity: 1,
          label: "President Share",
          percent: 20,
          shares: 2
        },
        {
          quantity: 8,
          percent: 10,
          shares: 1
        }
      ]
    },
    {
      name: "Sanuki Railway",
      abbrev: "SR",
      color: "blue",
      tokens: ["Free", "¥40"],
      shares: [
        {
          quantity: 1,
          label: "President Share",
          percent: 20,
          shares: 2
        },
        {
          quantity: 8,
          percent: 10,
          shares: 1
        }
      ]
    },
    {
      name: "Takamatsu & Kotohira Electric Railway",
      abbrev: "KO",
      color: "pink",
      tokens: ["Free", "¥40"],
      shares: [
        {
          quantity: 1,
          label: "President Share",
          percent: 20,
          shares: 2
        },
        {
          quantity: 8,
          percent: 10,
          shares: 1
        }
      ]
    },
    {
      name: "Tosa Electric Railway",
      abbrev: "TR",
      color: "darkGreen",
      tokens: ["Free", "¥40", "¥40"],
      shares: [
        {
          quantity: 1,
          label: "President Share",
          percent: 20,
          shares: 2
        },
        {
          quantity: 8,
          percent: 10,
          shares: 1
        }
      ]
    },
    {
      name: "Tosa Kuroshio Railway",
      abbrev: "KU",
      color: "purple",
      tokens: ["Free"],
      shares: [
        {
          quantity: 1,
          label: "President Share",
          percent: 20,
          shares: 2
        },
        {
          quantity: 8,
          percent: 10,
          shares: 1
        }
      ]
    },
    {
      name: "Uwajima Railway",
      abbrev: "UR",
      color: "maroon",
      tokens: ["Free", "¥40", "¥40"],
      shares: [
        {
          quantity: 1,
          label: "President Share",
          percent: 20,
          shares: 2
        },
        {
          quantity: 8,
          percent: 10,
          shares: 1
        }
      ]
    }
  ],

  phases: [
    {
      name: "2",
      limit: "4",
      number: "6",
      tiles: "yellow"
    },
    {
      name: "3",
      limit: "4",
      number: "5",
      tiles: "green",
      notes: "Private companies can be bought"
    },
    {
      name: "4",
      rust: "2",
      limit: "3",
      number: "4",
      tiles: "green"
    },
    {
      name: "5",
      limit: "2",
      number: "3",
      tiles: "brown",
      notes: "Private companies close"
    },
    {
      name: "6",
      limit: "2",
      number: "2",
      rust: "3",
      tiles: "brown",
      notes: "D Trains available for purchase"
    },
    {
      name: "D",
      limit: "2",
      number: "∞",
      rust: "4",
      tiles: "brown"
    }
  ],

  privates: [
    {
      name: "Champlain & St. Lawrence",
      price: "Min Bid $20",
      revenue: "$10",
      description: "Face Value $30."
    },
    {
      name: "Mitsubishi Ferry",
      price: "$30",
      revenue: "$5",
      description:
      "Player owner may place the port tile on a coastal town (B11, G10, I12, or J9) without a tile on it already, outside of the operating rounds of a company controller by another player. The player need not control a company or have connectivity to the placed tile from one of their companies. This does not close the company."
    },
    {
      name: "Ehime Railroad",
      price: "¥40",
      revenue: "¥10",
      description:
      "When this company is sold to a corporation, the selling player may immediately place a green tile on Ohzu (C4), in addition to any tile which it may lay during the same operating round. This does not close the company."
    },
    {
      name: "Sumitomo Mines Railway",
      price: "¥50",
      revenue: "¥15",
      description:
      "Owning corporation may ignore building cost for mountain hexes which do not also contain rivers. This does not close the company."
    },
    {
      name: "Dougo Railway",
      price: "¥60",
      revenue: "¥15",
      description:
      "Owning player may exchange this private company for a 10% share of Iyo Railway from the initial offering."
    },
    {
      name: "South Iyo Railway",
      price: "¥80",
      revenue: "¥20",
      players: "Players: 3+"
    },
    {
      name: "Uno-Takamatsu Ferry",
      price: "¥150",
      revenue: "¥30 / ¥50",
      players: "Players: 4+",
      description:
      "Does not close while owned by a player. If owned by a player when the first 5-train is purchased it may no longer be sold to a public company and the revenue is increased to 50."
    }
  ],

  tiles: {
    "15": 2,
    "70": 2,
    "120": 1,
    "122": 1,
    "124": 1,
    "X1": 1,
    "X2": 1,
    "X3": 1,
    "X4": 1,
    "X5": 1,
    "X6": 1,
    "X7": 1,
    "X8": 1,
  },

  map: {
    print: "landscape",
    hexes: [
      {
        color: "plain",
        hexes: ["B6", "B8", "C5", "C7", "C19", "D4", "D6", "D14", "E3", "E5", "E7", "E9", "F2", "F4", "F6", "F10", "F12", "F14", "G3", "G5", "G7", "G9", "G11", "G13", "H4", "H6", "H8", "H12", "I5", "I7", "I9", "I11", "I13", "J6", "J8", "J10", "J14", "K5", "K7", "K9", "L6", "L8", "M5", "M7", "N6", "O9", "O11"]
      },
      {
        color: "gray",
        values: [
          {
            angle: 210,
            percent: 0.8,
            value: 40
          }
        ],
        cities: [
          {
            size: 1,
            name: {
              name: "Timmins",
              rotation: -30,
              reverse: false
            },
            companies: [
              {
                color: "green"
              }
            ]
          }
        ],
        track: [{ side: 1 }, { side: 2 }, { side: 5 }, { side: 6 }],
        hexes: ["D2"]
      },
      {
        color: "plain",
        water: {
          cost: "$20"
        },
        hexes: ["K11"]
      },
      {
        color: "gray",
        track: [
          {
            type: "gentle",
            side: 5
          }
        ],
        hexes: ["C3"]
      },      
      {
        color: "gray",
        values: [ { angle: 180, percent: 0.6, value: 80 } ],
        labels: [
          {
            label: "Run to T, M, Q",
            angle: 180,
            percent: .15
          }
        ],
        track: [
          {
            type: "gentle",
            side: 6
          }
        ],
        hexes: ["E1"]
      },
      {
        color: "gray",
        track: [
          {
            type: "sharp",
            side: 6
          }
        ],
        hexes: ["B16"]
      },
      {
        color: "gray",
        track: [
          {
            type: "sharp",
            side: 3
          }
        ],
        hexes: ["L14"]
      },
      {
        color: "plain",
        cities: [
          {
            name: {
              name: "Sudbury"
            }
          }
        ],
        hexes: ["D8"]
      },
      {
        color: "plain",
        cities: [
          {
            
            name: {
              name: "North Bay"
            }
          }
        ],
        hexes: ["F8"]
      },
      {
        color: "plain",
        cities: [
          {
            
            name: {
              name: "Barrie"
            }
          }
        ],
        hexes: ["E13"]
      },
      {
        color: "plain",
        cities: [
          {
            
            name: {
              name: "Guelph"
            }
          }
        ],
        hexes: ["E15"]
      },
      {
        color: "plain",
        labels: [
          {
            label: "Y",
            angle: 330,
            percent: 0.8
          }
        ],
        cities: [
          {
            
            name: {
              name: "Hamilton"
            }
          }
        ],
        hexes: ["E17"]
      },
      {
        color: "plain",
        labels: [
          {
            label: "Y",
            angle: 330,
            percent: 0.8
          }
        ],
        cities: [
          {
            
            name: {
              name: "Berlin"
            }
          }
        ],
        hexes: ["D16"]
      },
      {
        color: "plain",
        cities: [
          {
            
            name: {
              name: "London"
            }
          }
        ],
        hexes: ["C17"]
      },
      {
        color: "plain",
        cities: [
          {
            
            name: {
              name: "Peterborough"
            }
          }
        ],
        hexes: ["G15"]
      },
      {
        color: "plain",
        cities: [
          {
            
            name: {
              name: "Kingston"
            }
          }
        ],
        hexes: ["I15"]
      },
      {
        color: "plain",
        water: {
          angle: 0,
          percent: 0.7,
          cost: "$20"
        },
        labels: [
          {
            label: "Y",
            angle: 270,
            percent: 0.7
          },
          {
            label: "O",
            angle: 90,
            percent: 0.7
          }
        ],
        cities: [
          {
            name: {
              name: "Ottawa"
            }
          }
        ],
        hexes: ["J12"]
      },
      {
        color: "plain",
        bridges: [{
          cost: "$80",
          angle: 0,
          percent: 1
        },
        {
          cost: "$80",
          angle: 300,
          percent: 1
        }],
        borders: [
            {
              side: 1,
              color: "water"
            },
            {
              side: 6,
              color: "water"
            }
          ],
        cities: [
          {
            
            name: {
              name: "Trois-Rivières"
            }
          }
        ],
        hexes: ["M9"]
      },
      {
        color: "plain",
        bridges: [{
          cost: "$80",
          angle: 300,
          percent: 1
        },
        {
          cost: "$80",
          angle: 0,
          percent: 1
        }],
        borders: [
            {
              side: 1,
              color: "water"
            },
            {
              side: 6,
              color: "water"
            }
          ],
        hexes: ["N8"]
      },
      {
        color: "plain",
        bridges: [{
          cost: "$80",
          angle: 120,
          percent: 1
        }],
        borders: [
            {
              side: 3,
              color: "water"
            },
          ],
        hexes: ["O9"]
      },
      {
        color: "plain",
        bridges: [{
          cost: "$80",
          angle: 180,
          percent: 1
        },
        {
          cost: "$80",
          angle: 120,
          percent: 1
        }],
        borders: [
            {
              side: 3,
              color: "water"
            },
            {
              side: 4,
              color: "water"
            },
          ],
        hexes: ["N10"]
      },
      {
        color: "plain",
        bridges: [{
          cost: "$80",
          angle: 180,
          percent: 1
        },
        {
          cost: "$80",
          angle: 120,
          percent: 1
        }],
        borders: [
            {
              side: 3,
              color: "water"
            },
            {
              side: 4,
              color: "water"
            },
          ],
        hexes: ["M11"]
      },
      {
        color: "plain",
        labels: [
          {
            label: "Y",
            angle: 330,
            percent: 0.8
          }
        ],
        cities: [
          {
            
            name: {
              name: "Quebec"
            }
          }
        ],
        hexes: ["O7"]
      },
      {
        color: "plain",
        cities: [
          {
            
            name: {
              name: "Sherbrooke"
            }
          }
        ],
        hexes: ["N12"]
      },
      {
        color: "plain",
        centerTowns: [
          {
            name: {
              name: "Goderich"
            }
          }
        ],
        hexes: ["C15"]
      },
      {
        color: "plain",
        centerTowns: [
          {
            name: {
              name: "Sarnia"
            }
          }
        ],
        hexes: ["B18"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 1,
              color: "water"
            }
          ],
          centerTowns: [
          {
            name: {
              name: "Belleville"
            }
          }
        ],
        hexes: ["H14"]
      },
      {
        color: "plain",
        centerTowns: [
          {
            name: {
              name: "Pembroke"
            }
          }
        ],
        hexes: ["H10"]
      },
      {
        color: "plain",
        centerTowns: [
          {
            name: {
              name: "Cornwall"
            }
          }
        ],
        hexes: ["K13"]
      },
      {
        color: "plain",
        bridges: [{
          cost: "$80",
          angle: 300,
          percent: 1
        }],
        borders: [
            {
              side: 6,
              color: "water"
            }
          ],
        centerTowns: [
          {
            name: {
              name: "St. Jerome"
            }
          }
        ],
        hexes: ["L10"]
      },
      {
        color: "plain",
        companies: [
          {
            label: "C&StL",
            percent: 0.5
          }
        ],
        centerTowns: [
          {
            name: {
              name: "Granby"
            }
          }
        ],
        hexes: ["M13"]
      },
      {
        color: "yellow",
        track: [
          {
            side: 2,
            type: "stop"
          },
          {
            side: 4,
            type: "stop"
          }
        ],
        labels: {
          angle: 130,
          percent: 0.75,
          label: "M"
        },
        values: [ { angle: 240, percent: 0.70, value: 40 } ],
        water: {cost: "$20", angle: 0, percent: 0.8},
        cities: [
          {
            angle: 60,
            percent: 0.5,
            name: {
              name: "Montreal"

            },
          },
          {
            angle: 180,
            percent: 0.5,
            companies: [
              {              }
            ]
          },
          {
            angle: 300,
            percent: 0.5,
            companies: [
              {
                color: "green"
              }
            ]
          },
        ],
        hexes: ["L12"]
      },
      {
        color: "yellow",
        track: [
          {
            side: 2,
            type: "stop"
          },
          {
            side: 5,
            type: "stop"
          }
        ],
        labels: {
          angle: 150,
          percent: 0.75,
          label: "T"
        },
        values: [ { angle: 330, percent: 0.70, value: 30 } ],
        cities: [
          {
            angle: 60,
            percent: 0.5,
            name: {
              name: "Toronto"

            },
            companies: [
              {
                label: "CN",
                color: "white"
              }
            ]
          },
          {
            angle: 240,
            percent: 0.5,
            companies: [
              {              }
            ]
          },
          
        ],
        hexes: ["F16"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 5}, {side: 6}],
        offBoardRevenue: {
          name: {name: "Sault Ste. Marie"},
          angle: 90,
          percent: 0.3,
          revenues: [
            {
              color: "yellow",
              cost: "20"
            },
            {
              color: "green",
              cost: "30"
            },
            {
              color: "brown",
              cost: "40"
            },
            {
              color: "gray",
              cost: "40"
            },
          ]
        },
        hexes: ["A7"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 3}],
        offBoardRevenue: {
          name: {name: "Buffalo"},
          angle: 300,
          percent: 0.25,
          revenues: [
            {
              color: "yellow",
              cost: "30"
            },
            {
              color: "green",
              cost: "40"
            },
            {
              color: "brown",
              cost: "50"
            },
            {
              color: "gray",
              cost: "60"
            },
          ]
        },
        hexes: ["F18"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 1,
              color: "water"
            },
            {
              side: 6,
              color: "water"
            }
          ],
        hexes: ["C9"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 1,
              color: "water"
            },
            {
              side: 2,
              color: "water"
            },
            {
              side: 3,
              color: "water"
            },
            {
              side: 6,
              color: "water"
            }
          ],
        hexes: ["D10"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 2,
              color: "water"
            },
            {
              side: 3,
              color: "water"
            },
          ],
        hexes: ["E11"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 4,
              color: "water"
            },
            {
              side: 1,
              color: "water"
            },
            {
              side: 5,
              color: "water"
            },
          ],
        hexes: ["C11"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 4,
              color: "water"
            },
            {
              side: 5,
              color: "water"
            },
          ],
        hexes: ["D12"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 4,
              color: "water"
            },
          ],
        hexes: ["C13"]
      },
      {
        color: "plain",
        borders: [
            {
              side: 6,
              color: "water"
            },
          ],
        hexes: ["D18"]
      },
      {
        color: "water",
        icons: [
          {
            type: "port",
            percent: 0.6
          }
        ],
        offBoardTrack: [{side: 4},],values: [
          {
            value: 10,
            angle: 270,
            percent: 0
          }
        ],
        borders: [
            {
              side: 3,
              color: "water"
            },
          ],
        hexes: ["E19"]
      },
      {
        color: "water",
        icons: [
          {
            type: "port",
            percent: 0.6
          }
        ],
        offBoardTrack: [{side: 3},{side: 5}],values: [
          {
            value: 10,
            angle: 270,
            percent: 0
          }
        ],
        borders: [
            {
              side: 4,
              color: "water"
            },
          ],
        hexes: ["H16"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 4}],
        offBoardRevenue: {
          name: {name: "New England"},
          angle: 0,
          percent: 0.25,
          revenues: [
            {
              color: "yellow",
              cost: "30"
            },
            {
              color: "green",
              cost: "40"
            },
            {
              color: "brown",
              cost: "50"
            },
            {
              color: "gray",
              cost: "60"
            },
          ]
        },
        hexes: ["M15"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 3},{side: 4}],
        offBoardRevenue: {
          name: {name: "Maine"},
          angle: 300,
          percent: 0.2,
          revenues: [
            {
              color: "yellow",
              cost: "20"
            },
            {
              color: "green",
              cost: "30"
            },
            {
              color: "brown",
              cost: "40"
            },
            {
              color: "gray",
              cost: "40"
            },
          ]
        },
        hexes: ["O13"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 3},{side: 2}],
        offBoardRevenue: {
          name: {name: "Maritime Provinces"},
          angle: 270,
          percent: 0.3,
          revenues: [
            {
              color: "yellow",
              cost: "20"
            },
            {
              color: "green",
              cost: "30"
            },
            {
              color: "brown",
              cost: "40"
            },
            {
              color: "gray",
              cost: "40"
            },
          ]
        },
        hexes: ["P8"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 6}],
        
        hexes: ["A17"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 5}],
        offBoardRevenue: {
          name: {name: "Detroit"},
          rows: 4,
          angle: 180,
          percent: 1.4,
          revenues: [
            {
              color: "yellow",
              cost: "30"
            },
            {
              color: "green",
              cost: "40"
            },
            {
              color: "brown",
              cost: "50"
            },
            {
              color: "gray",
              cost: "70"
            },
          ]
        },
        hexes: ["A19"]
      },
      /*
      {
        color: "plain",
        cities: [
          {
            name: {
              name: "Saijou"
            }
          }
        ],
        hexes: ["F3"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Niihama"
            }
          }
        ],
        hexes: ["G4"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Ikeda"
            }
          }
        ],
        hexes: ["H7"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Sukumo"
            }
          }
        ],
        hexes: ["A10"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Anan"
            }
          }
        ],
        hexes: ["J11"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Nahari"
            }
          }
        ],
        hexes: ["G12"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Matsuyama"
            },
            companies: [
              {
                label: "IR",
                color: "orange"
              }
            ]
          }
        ],
        hexes: ["E2"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Marugame"
            },
            companies: [
              {
                label: "SR",
                color: "blue"
              }
            ]
          }
        ],
        hexes: ["I2"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Tokushima"
            },
            companies: [
              {
                label: "AR",
                color: "red"
              }
            ]
          }
        ],
        hexes: ["K8"]
      },
      {
        copy: "F3",
        cities: [
          {
            name: {
              name: "Kubokawa"
            },
            companies: [
              {
                label: "KU",
                color: "purple"
              }
            ]
          }
        ],
        hexes: ["C10"]
      },
      {
        color: "plain",
        centerTowns: [
          {
            name: {
              name: "Ritsurin Kouen"
            }
          }
        ],
        hexes: ["J5"]
      },
      {
        color: "plain",
        icons: [
          {
            type: "port",
            percent: 0.5
          }
        ],
        centerTowns: [
          {
            name: {
              name: "Nangoku"
            }
          }
        ],
        hexes: ["G10"]
      },
      {
        color: "plain",
        icons: [
          {
            type: "port",
            percent: 0.5
          }
        ],
        centerTowns: [
          {
            name: {
              name: "Komatsujima"
            }
          }
        ],
        hexes: ["J9"]
      },
      {
        color: "plain",
        icons: [
          {
            type: "port",
            percent: 0.5
          }
        ],
        centerTowns: [
          {
            name: {
              name: "Muki"
            }
          }
        ],
        hexes: ["I12"]
      },
      {
        color: "plain",
        icons: [
          {
            type: "port",
            percent: 0.5
          }
        ],
        centerTowns: [
          {
            name: {
              name: "Nakamura"
            }
          }
        ],
        hexes: ["B11"]
      },
      {
        color: "plain",
        water: {
          cost: "80"
        },
        hexes: ["K6"]
      },
      {
        color: "plain",
        water: {percent: 0.38},
        mountain: {
          angle: 180,
          percent: 0.1,
          cost: "80"
        },
        hexes: ["H5", "I6"]
      },
      {
        color: "plain",
        mountain: {
          cost: "80"
        },
        hexes: [
          "E4",
          "D5",
          "F5",
          "C6",
          "E6",
          "G6",
          "D7",
          "F7",
          "A8",
          "G8",
          "B9",
          "H9",
          "H11",
          "H13"
        ]
      },
      {
        color: "plain",
        cities: [
          {
            angle: 120,
            percent: 0.4,
            name: {
              name: "Kotohira",
              rotation: 30
            }
          }
        ],
        mountain: {
          angle: 270,
          percent: 0.5,
          cost: "80"
        },
        labels: [
          {
            label: "H",
            angle: 330,
            percent: 0.8
          }
        ],
        hexes: ["I4"]
      },
      {
        color: "yellow",
        values: [
          {
            value: 20,
            angle: 180,
            percent: 0.667
          }
        ],
        cities: [
          {
            name: {
              name: "Ohzu",
              reverse: true
            }
          }
        ],
        companies: [
          {
            label: "ER",
            bottom: true,
            angle: 270,
            percent: 0.667
          }
        ],
        track: [
          {
            side: 3
          }
        ],
        hexes: ["C4"]
      },
      {
        copy: "C4",
        labels: [
          {
            label: "T",
            angle: 330,
            percent: 0.8
          }
        ],
        cities: [
          {
            name: {
              name: "Takamatsu",
              reverse: false,
              rotation: 60
            },
            companies: [
              {
                label: "KO",
                color: "pink"
              }
            ]
          }
        ],
        values: [
          {
            value: 30,
            angle: 150,
            percent: 0.8
          }
        ],
        companies: [
          {
            label: "TR",
            percent: 0.75
          }
        ],
        track: [
          {
            side: 1
          },
          {
            side: 2
          }
        ],
        hexes: ["K4"]
      },
      {
        color: "gray",
        values: [
          {
            angle: 210,
            percent: 0.8,
            value: 40
          }
        ],
        cities: [
          {
            size: 2,
            name: {
              name: "Uwajima",
              reverse: true
            },
            companies: [
              {
                label: "UR",
                color: "maroon"
              }
            ]
          }
        ],
        track: [{ side: 2 }, { side: 4 }, { side: 6 }],
        hexes: ["B7"]
      },
      {
        color: "gray",
        towns: [
          {
            angle: -30,
            percent: 0.57735,
            rotation: 60,
            name: {
              name: "Yawatahama"
            }
          }
        ],
        track: [
          {
            type: "sharp",
            side: 6
          }
        ],
        values: [
          {
            value: 20,
            angle: 150,
            percent: 0.2
          }
        ],
        hexes: ["B3"]
      },
      {
        color: "gray",
        towns: [
          {
            angle: -150,
            percent: 0.57735,
            rotation: 120,
            name: {
              name: "Muroto",
              reverse: true
            }
          }
        ],
        track: [
          {
            type: "sharp",
            side: 4
          }
        ],
        values: [
          {
            value: 20,
            angle: 30,
            percent: 0.2
          }
        ],
        hexes: ["G14"]
      },
      {
        color: "gray",
        track: [
          {
            type: "gentle",
            side: 6
          }
        ],
        hexes: ["J7"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 1}, {side: 2}],
        offBoardRevenue: {
          name: {name: "Imabari"},
          revenues: [
            {
              color: "yellow",
              cost: "30"
            },
            {
              color: "brown",
              cost: "60"
            },
            {
              color: "black",
              textColor: "white",
              cost: "D100"
            }
          ]
        },
        hexes: ["F1"]
      },
      {
        copy: "F1",
        offBoardRevenue: {
          name: {name: "Sakaide & Okayama"},
          revenues: [
            {
              color: "yellow",
              cost: "20"
            },
            {
              color: "brown",
              cost: "40"
            },
            {
              color: "black",
              textColor: "white",
              cost: "D80"
            }
          ]
        },
        hexes: ["J1"]
      },
      {
        color: "offboard",
        offBoardTrack: [{side: 2}, {side: 3}],
        offBoardRevenue: {
          name: {name: "Naruto & Awaji"},
          x: 20,
          revenues: [
            {
              color: "yellow",
              cost: "20"
            },
            {
              color: "brown",
              cost: "40"
            },
            {
              color: "black",
              textColor: "white",
              cost: "D80"
            }
          ]
        },
        hexes: ["L7"]
      },
      {
        color: "green",
        water: {cost: 80, angle: 90, percent: 0.8},
        values: [{
          angle: 150,
          percent: 0.82,
          value: 30
        }],
        labels: [{
          label: "K",
          angle: 270,
          percent: 0.82
        }],
        cities: [
          {
            size: 2,
            rotation: 120,
            name: {
              name: "Kouchi",
              reverse: true,
              offset: 90
            },
            companies: [
              {
                label: "TR",
                color: "darkGreen"
              }
            ]
          }
        ],
        track: [{ side: 3 }, { side: 4 }, { side: 5 }, { side: 6 }],
        hexes: ["F9"]
      }
      */
    ]
  }
};

export default game;
