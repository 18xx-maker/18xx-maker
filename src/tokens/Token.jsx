        ...
        if (inverse && logo && logos[logo]) {
          ...
          if (logo.includes("countries")) {
            shapes.push(
              <Component
                key="logo"
                className={`color-main-${color}${reserved ? " color-reserved" : ""}`}
                x={start}
                y={start}
                preserveAspectRatio="xMidYMid slice"
                height={size}
          ...
          if (sunrise) {
            shapes.push(
              <g key="sunrise" transform={`rotate(${shapeAngle || 0})`}>
                <rect
                  key="upper"
                  x="-50"
                  y="-50"
                ...
                <polygon
                  key="sunrise-1"
                  points="0 0 35.35534 -35.35534 12.94095 -49.29629"
                  fill={c(sunrise[2])}
                  clipPath={`url(#${clipId})`}
                />
                <polygon
          ...
          if (sexies) {
            shapes.push(
              <g key="sexies" transform={`rotate(${shapeAngle || 0})`}>
                <polygon
                  key="sexies-1"
                  points="0 0 50 0 25 43.30127"
                  fill={c(sexies[0])}
                  clipPath={`url(#${clipId})`}
                />
                <polygon
                  key="sexies-2"
                  points="0 0 25 43.30127 -25 43.30127"
                  fill={c(sexies[1])}
                  clipPath={`url(#${clipId})`}
                />
                <polygon
                  key="sexies-3"
                  points="0 0 -25 43.30127 -50 0"
                  fill={c(sexies[0])}
                  clipPath={`url(#${clipId})`}
                />
                <polygon
          ...
          if (hexagram) {
            ...
            shapes.push(
              <g key="hexagram" transform={`rotate(${shapeAngle || 0})`}>
                <polygon
                  points={`${hexagramSide} -${hexagramBottom} -${hexagramSide} -${hexagramBottom} 0 ${hexagramTop}`}
                  fill={c(hexagram[1])}
                  clipPath={`url(#${clipId})`}
                />
                <polygon
                ...
                {(cityRotateContext) => (
                  ...
                  >
                    ...
                    <g clipPath={`url(#${clipId})`}>
                      <g transform={`rotate(${shapeAngle || 0})`}>{tokInner}</g>
                      {shapes}
                      {content}
                    </g>
                    {tokOuter}
